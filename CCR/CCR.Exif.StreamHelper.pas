...
{$IFEND}
  ...
  TStreamHelper = class helper for TStream
    ...
    function TryReadHeader(const Header; HeaderSize: Byte; AlwaysResetPosition: Boolean = False): Boolean;
...
function TStreamHelper.ReadLongWord(Endianness: TEndianness; var Value: LongWord): Boolean;
begin
  Result := TryReadBuffer(Value, 4);
  if Result and (Endianness = BigEndian) then
...
function TStreamHelper.ReadSmallInt(Endianness: TEndianness; var Value: SmallInt): Boolean;
begin
  Result := TryReadBuffer(Value, 2);
  if Result and (Endianness = BigEndian) then
    Value := Swap(Value)
...
begin
  ...
  if not Result then
    Seek(-BytesRead, soCurrent);
end;

function TStreamHelper.TryReadHeader(const Header; HeaderSize: Byte; AlwaysResetPosition: Boolean = False): Boolean;
var
...
begin
  ...
  if not Result or AlwaysResetPosition then
    Seek(-BytesRead, soCurrent);
end;
...
procedure TStreamHelper.WriteUTF8Chars(const S: UnicodeString; const Args: array of const);
begin
