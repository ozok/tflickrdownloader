...
procedure DoSaveToGraphic(const FileName: string; const DetectTypeFunc: TGetGraphicSaveMethod); overload;
...
type
  ...
  TAdobeResBlock = record
  public const
    ExifTypeID = $0422;
    IPTCTypeID = $0404;
    XMPTypeID = $0424;
    NewIPTCTagMarker: Byte = 28;
    class function TryReadIPTCHeader(Stream: TStream; var TagInfo: TIPTCTagInfo; AlwaysResetStreamPos: Boolean = False): Boolean; static;
  end;
...
{$Z2}
  ...
  TAdobeColorMode = (cmMonochrome, cmGrayscale, cmIndexed, cmRGB, cmCMYK, cmMultichannel, cmHalftone, cmLAB);
{$Z1}
...
function CreateAdobeBlock(ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist = nil): IAdobeResBlock; overload;
function CreateAdobeBlock(const ASignature: AnsiString; ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist = nil): IAdobeResBlock; overload;

function HasPSDHeader(Stream: TStream): Boolean; overload;
function HasPSDHeader(const FileName: string): Boolean; overload;

function ParsePSDHeader(Stream: TStream; var Info: TPSDInfo; StreamOwnership: TStreamOwnership = soReference): IAdobeResBlockParser; overload;
...
type
  ...
  public
    ...
    case Integer of
      0:
        (DataSizeHi, DataSizeLo: Byte;
          Data: record end);
      1:
        (DataSizeBigEndian: Word);
  end;
  ...
  TUserJPEGSegment = class(TMetadataBlock, IMetadataBlock, IJPEGSegment, IStreamPersist, IStreamPersistEx)
  strict private
...
function JPEGHeader(JPEGStream: TStream; const MarkersToLookFor: TJPEGMarkers = TJPEGSegment.AnyMarker; StreamOwnership: TStreamOwnership = soReference): IJPEGHeaderParser; overload;
function JPEGHeader(const JPEGFile: string; const MarkersToLookFor: TJPEGMarkers = TJPEGSegment.AnyMarker): IJPEGHeaderParser; overload; inline;
...
procedure UpdateApp1JPEGSegments(InStream, OutStream: TStream; NewExifSource, NewXMPSource: IStreamPersistEx);
// used by the SaveToStream methods of TExifData and TXMPPacket; if NewXXXSource is nil, the data in InStream is copied over

procedure WriteJPEGFileHeader(Stream: TStream); inline;
procedure WriteJPEGSegment(Stream: TStream; MarkerNum: TJPEGMarker; const Data; DataSize: Word); overload;
procedure WriteJPEGSegment(Stream: TStream; MarkerNum: TJPEGMarker; Data: TStream; DataSize: Word = 0); overload;
...
begin
  if (Windows.GetTempPath(MAX_PATH, Buffer) = 0) or (Windows.GetTempFileName(Buffer, 'tmp', 0, Buffer) = 0) then
    RaiseLastOSError
  else
    Result := Buffer;
end;
...
procedure DoSaveToGraphic(GraphicStream: TStream; const DetectTypeFunc: TGetGraphicSaveMethod); overload;
var
...
procedure DoSaveToGraphic(const FileName: string; const DetectTypeFunc: TGetGraphicSaveMethod);
var
  Stream: TFileStream;
begin
...
procedure DoSaveToGraphic(const Graphic: IStreamPersist; const DetectTypeFunc: TGetGraphicSaveMethod);
var
...
begin
  ...
  if BytesToRead > 0 then
    FData.CopyFrom(DataSource, BytesToRead);
end;
...
begin
  Result := False;
  if (CheckID and not HasExifBlockID) or (FData.Size < 8) then
    Exit;
  Words := FData.Memory;
  ...
  if (Words[0] = TiffSmallEndianCode) or (Words[0] = TiffBigEndianCode) then
    case Words[1] of
      TiffMagicNum, TiffMagicNumBigEndian:
        Result := True; // ParseTiff doesn't mind if these are confused
    end;
end;
...
begin
  ...
  Result := (not CheckID or HasIPTCBlockID) and TAdobeResBlock.TryReadIPTCHeader(FData, Header, True);
end;
...
begin
  Result := False;
  if (CheckID and not HasXMPBlockID) or (FData.Size < 20) then
    Exit;
...
begin
  Result := (A.MissingOrInvalid and B.MissingOrInvalid) or ((A.Value <= B.Value) and not A.MissingOrInvalid and not B.MissingOrInvalid);
end;
...
class function TAdobeResBlock.TryReadIPTCHeader(Stream: TStream; var TagInfo: TIPTCTagInfo; AlwaysResetStreamPos: Boolean = False): Boolean;
var // IPTC data is made up of a continuous stream of tags that just begin.
  Header: packed record StartMarker: Byte;
  RecID: Byte;
  TagID: Byte;
  DataSize: SmallInt;
end;
...
begin
  Result := False;
  if not Stream.TryReadBuffer(Header, SizeOf(Header)) then
    Exit;
  ...
  begin
    ...
    if Header.DataSize >= 0 then
      ...
      case Header.DataSize of
        - 1:
          if Stream.ReadByte(ByteVal) then
            TagInfo.DataSize := ByteVal
          else
            Result := False;
        -2:
          if Stream.ReadWord(BigEndian, WordVal) then
            TagInfo.DataSize := WordVal
          else
            Result := False;
        -4:
          if Stream.ReadLongInt(BigEndian, LongIntVal) then
...
constructor TAdobeBlock.Create(const ASignature: AnsiString; ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist);
begin
  inherited Create;
  FLoadedCleanly := True;
...
var // Method amended in trunk r.14 to avoid exceptions on some malformed
  ...
  Header: packed record Signature: array [0 .. 3] of AnsiChar;
...
begin
  ...
  if Header.NameLen <> 0 then
    if not Stream.TryReadBuffer(Pointer(FName)^, Header.NameLen) then
      Exit;
  if not Odd(Header.NameLen) then
    if Stream.Read(Len, 1) <> 1 then
      Exit;
  if not Stream.ReadLongInt(BigEndian, Len) or (Len < 0) then
    Exit;
  if Len <> 0 then
  begin
    Data.SetSize(Len);
    if not Stream.TryReadBuffer(Data.Memory^, Len) then
      Exit;
    if Odd(Len) then
      if Stream.Read(Len, 1) <> 1 then
        Exit;
...
begin
  ...
  if Len <> 0 then
    Stream.WriteBuffer(Data.Memory^, Len);
  if Odd(Len) then
    Stream.WriteByte(0);
end;
...
constructor TAdobeResBlockEnumerator.Create(AStream: TStream; AStreamOwnership: TStreamOwnership);
begin
  inherited Create;
  FStream := AStream;
...
constructor TUserJPEGSegment.Create(AMarkerNum: TJPEGMarker = 0; const ASource: IStreamPersist = nil);
begin
  inherited Create;
...
begin
  Result := False;
  FAdobeHeaderSize := 0;
  if (MarkerNum <> jmApp13) or (Data.Size < 20) then
    Exit;
...
begin
  ...
  for ResBlock in Self do
    if ResBlock.IsIPTCBlock(CheckID) then
      Exit;
  Result := False;
end;
...
constructor TFoundJPEGSegment.Create(AMakerNum: TJPEGMarker; ASource: TStream; ADataSize: Integer);
begin
...
begin
  ...
  if MarkerNum in TJPEGSegment.MarkersWithNoData then
    Dec(Result, 2);
end;

{ TJPEGHeaderParser }

constructor TJPEGHeaderParser.Create(JPEGStream: TStream; const MarkersToLookFor: TJPEGMarkers; StreamOwnership: TStreamOwnership);
begin
  inherited Create;
...
begin
  FCurrent := nil;
  Result := False;
  if (FStream = nil) or (FLastMarker = jmEndOfImage) then
    Exit;
  ...
  begin
    ...
    try
      if AllocatedBuffer then
        FStream.ReadBuffer(Buffer^, BufferSize);
      MaxPos := @Buffer[BufferSize - 1];
      SeekPtr := Buffer;
      while SeekPtr < MaxPos do
      begin
        Inc(SeekPtr);
        if Byte(SeekPtr^) <> jmNewOrPadding then
          Continue;
        Inc(SeekPtr);
        if Byte(SeekPtr^) <> jmEndOfImage then
          Continue;
        Inc(SeekPtr);
...
function JPEGHeader(JPEGStream: TStream; const MarkersToLookFor: TJPEGMarkers; StreamOwnership: TStreamOwnership): IJPEGHeaderParser; overload;
begin
  Result := TJPEGHeaderParser.Create(JPEGStream, MarkersToLookFor, StreamOwnership);
end;

function JPEGHeader(const JPEGFile: string; const MarkersToLookFor: TJPEGMarkers): IJPEGHeaderParser;
begin
  Result := JPEGHeader(TFileStream.Create(JPEGFile, fmOpenRead or fmShareDenyWrite), MarkersToLookFor, soOwned);
end;

function JPEGHeader(const JPEGImage: IStreamPersist; const MarkersToLookFor: TJPEGMarkers): IJPEGHeaderParser;
var
  Stream: TMemoryStream;
begin
  Stream := TMemoryStream.Create;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FoundExif := True;
        if NewExifSource = nil then
          NewExifSource := IStreamPersistEx(Segment);
      end
      else if not FoundXMP and Segment.IsXMPBlock then
      begin
        FoundXMP := True;
        if NewXMPSource = nil then
          NewXMPSource := IStreamPersistEx(Segment);
      end
      else
        Continue;
    end;
...
procedure WriteJPEGSegment(Stream: TStream; MarkerNum: TJPEGMarker; Data: TStream; DataSize: Word); overload;
var
  Buffer: Pointer;
...
begin
  ...
  try
    if BufferAllocated then
      Data.ReadBuffer(Buffer^, DataSize);
...
function CreateAdobeBlock(ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist): IAdobeResBlock; overload;
begin
  Result := TAdobeBlock.Create('8BIM', ATypeID, AName, ADataSource);
end;

function CreateAdobeBlock(const ASignature: AnsiString; ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist): IAdobeResBlock; overload;
begin
...
constructor TAdobeResBlockParser.Create(AStream: TStream; AStreamOwnership: TStreamOwnership; var Info: TPSDInfo);
...
begin
  ...
  Result := (BytesRead = FixedPSDHeaderSize) and CompareMem(@Buffer, @FixedPSDHeader, FixedPSDHeaderSize);
...
function ParsePSDHeader(Stream: TStream; var Info: TPSDInfo; StreamOwnership: TStreamOwnership): IAdobeResBlockParser;
begin
