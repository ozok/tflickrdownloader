...
{$Z2}
  TTiffDataType = (tdByte = 1, tdAscii, tdWord, tdLongWord, tdLongWordFraction, tdShortInt, tdUndefined, tdSmallInt, tdLongInt, tdLongIntFraction, tdSingle, tdDouble, tdSubDirectory);
{$Z1}
  ...
  const
    ...
    class function MatchOffsetToByteCountID(OffsetTagID: TTiffTagID; var ByteCountID: TTiffTagID): Boolean; static;
  end;
  ...
  ITiffDirectory = interface // implemented by both the internal TFoundTiffDirectory below and TExifSection in CCR.Exif.pas
    ...
    function TagExists(TagID: TTiffTagID; ValidDataTypes: TTiffDataTypes = [Low(TTiffDataType) .. High(TTiffDataType)]; MinElementCount: LongInt = 1; MaxElementCount: LongInt = MaxLongInt): Boolean;
  ...
  TTiffParserEnumerator = {$IFDEF NoRecEnumBug} record {$ELSE} class sealed{$ENDIF}
      strict private FCurrent: IFoundTiffDirectory;
    FMaxPossibleOffset: Int64;
    FNextIndex: Integer;
    FNextOffset: LongInt;
    FSource: ITiffParser;
  public
    constructor Create(const ASource: ITiffParser; const AMaxPossibleOffset: Int64; const ANextOffset: LongInt);
    function MoveNext: Boolean;
  ...
  ITiffParser = interface
    ...
    function ParseSubDirectory(const OffsetTag: ITiffTag; out Directory: IFoundTiffDirectory): Boolean; overload;
    function ParseSubDirectory(const Parent: ITiffDirectory; const OffsetTag: TTiffTagInfo; out Directory: IFoundTiffDirectory): Boolean; overload;
  ...
  protected type
    ...
    public
      ...
      constructor Create(AParent: TTiffDirectoryRewriter; AID: TTiffTagID; ADataType: TTiffDataType; AElementCount: LongInt; AOriginalDataOffset: LongWord = 0); overload;
  ...
  protected
    ...
    procedure Write(AStream: TStream; const ABasePosition: Int64; AEndianness: TEndianness; ANextIFDOffset: LongWord);
  ...
  public
    ...
    procedure AddTag(ID: TTiffTagID; const DataSource: IStreamPersist; DataType: TTiffDataType = tdUndefined); overload;
    procedure AddTag(const Source: ITiffTag); overload;
    procedure AddSubDirectory(OffsetID: TTiffTagID; const SubDirectory: ITiffDirectory; const Callback: ITiffRewriteCallback = nil);
...
const
  TiffElementSizes: array [TTiffDataType] of Integer = (1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4, 8, 4);

function HasTiffHeader(Stream: TStream; var Endianness: TEndianness; MoveOnSuccess: Boolean = False): Boolean; overload;
...
function ParseTiffDirectory(Stream: TStream; Endianness: TEndianness; const BasePosition, Offset, InternalOffset: Int64): IFoundTiffDirectory;
...
type
  ...
  public
    ...
    constructor Create(AID: TTiffTagID; ADataType: TTiffDataType; const ADataSource: IStreamPersist); overload;
    destructor Destroy; override;
    property DataType: TTiffDataType read GetDataType;
    property ElementCount: LongInt read GetElementCount;
    property ID: TTiffTagID read GetID;
  ...
  protected
    ...
    constructor Create(const AParser: ITiffParser; const AParent: ITiffDirectory; AIndex: Integer; const Tags: TTiffTagInfoDynArray; const LoadErrors: TMetadataLoadErrors); overload;
    function GetEnumerator: ITiffDirectoryEnumerator;
    function GetIndex: Integer;
    function GetLoadErrors: TMetadataLoadErrors;
    function GetParent: ITiffDirectory;
  ...
  public
    constructor Create(const AParser: ITiffParser; const AParent: ITiffDirectory; AIndex: Integer; const AOffset: Int64; const AInternalOffset: Int64 = 0); overload;
    class function TryCreate(const AParser: ITiffParser; const AParent: ITiffDirectory; AIndex: Integer; const AOffset: Int64; out Instance: IFoundTiffDirectory): Boolean; static;
  ...
  public
    ...
    constructor Create(AStream: TStream; const ABasePosition: Int64; AEndianness: TEndianness); overload;
    destructor Destroy; override;
  ...
  public
    constructor Create(AParent: TTiffDirectoryRewriter; const AParser: ITiffParser; const ATagInfo: TTiffTagInfo);
  ...
  public
    ...
    constructor Create(AOwner: TTiffDirectoryRewriter; AID: TTiffTagID; ADataType: TTiffDataType; const ADataSource: IStreamPersist); overload;
    procedure WriteData(Stream: TStream; Endianness: TEndianness); override;
  ...
  public
    constructor Create(AOwner: TTiffDirectoryRewriter; AOffsetID: TTiffTagID; ADirectory: TTiffDirectoryRewriter);
    destructor Destroy; override;
    procedure WriteData(Stream: TStream; Endianness: TEndianness); override;
...
class function TTiffTag.MatchOffsetToByteCountID(OffsetTagID: TTiffTagID; var ByteCountID: TTiffTagID): Boolean;
begin
  Result := True;
  case OffsetTagID of
    ttStripOffsets:
      ByteCountID := ttStripByteCounts;
    ttTileOffsets:
      ByteCountID := ttTileOffsets;
    ttThumbnailOffset:
      ByteCountID := ttThumbnailSize;
  else
    Result := False;
  end;
end;
...
begin
  ...
  if DivSignPos <> 0 then
    Result := TryStrToInt(Copy(AString, 1, DivSignPos - 1), Numerator) and TryStrToInt(Copy(AString, DivSignPos + 1, MaxInt), Denominator)
  else
  begin
...
begin
  DivSignPos := Pos('/', AString);
  if DivSignPos <> 0 then
    Result := TryStrToLongWord(Copy(AString, 1, DivSignPos - 1), Numerator) and TryStrToLongWord(Copy(AString, DivSignPos + 1, MaxInt), Denominator)
  else
  begin
...
function LoadTiffDirectory(Stream: TStream; Endianness: TEndianness; const BasePosition, Offset, InternalOffset: Int64; out LoadErrors: TMetadataLoadErrors): TTiffTagInfoDynArray; overload;
var
  I, TagCount: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Offset64 < 8) or (Offset64 + BasePosition < 0) or (Offset64 + Result[I].DataSize + BasePosition > StreamSize) then
...
function LoadTiffDirectory(const Parser: ITiffParser; const Offset: Int64; out LoadErrors: TMetadataLoadErrors): TTiffTagInfoDynArray; overload; inline;
begin
  Result := LoadTiffDirectory(Parser.Stream, Parser.Endianness, Parser.BasePosition, Offset, 0, LoadErrors);
end;

function InitLoadOffsetArray(DataType: TTiffDataType; ElementCount: LongInt; out ArrayResult: TLongWordDynArray): Boolean; inline;
begin
...
function LoadOffsetArray(const Source: ITiffParser; const TagInfo: TTiffTagInfo): TLongWordDynArray; overload;
var
  WordArray: TWordDynArray;
  I: Integer;
begin
  if not InitLoadOffsetArray(TagInfo.DataType, TagInfo.ElementCount, Result) then
    Exit;
...
var
  ...
  I: LongInt;
begin
  if not InitLoadOffsetArray(Tag.DataType, Tag.ElementCount, Result) then
    Exit;
...
function SeekToExifThumbnail(const Parser: ITiffParser; const OffsetTag: TTiffTagInfo): Boolean;
begin
  Result := False;
  if not(OffsetTag.DataType in TTiffTag.OffsetDataTypes) or (OffsetTag.ElementCount <> 1) then
    Exit;
  Parser.Stream.Position := Parser.BasePosition + LoadOffsetArray(Parser, OffsetTag)[0];
  if HasJPEGHeader(Parser.Stream) then
    Result := True;
end;

procedure WriteTiffTagData(DataType: TTiffDataType; ElementCount: Longint; DataPtr: Pointer; Dest: TStream; Endianness: TEndianness);
var
  I, DataSize: Integer;
begin
  ...
  if DataType = tdDouble then
    ...
    case TiffElementSizes[DataType] of
      1:
        Dest.WriteBuffer(DataPtr^, ElementCount);
      2:
        for I := 0 to ElementCount - 1 do
...
constructor TFoundTiffTag.Create(AID: TTiffTagID; ADataType: TTiffDataType; const ADataSource: IStreamPersist);
begin
  inherited Create;
  New(FSourcePtr);
  FSourcePtr.ID := AID;
...
begin
  if (FParent = nil) and (FSourcePtr <> nil) then
    Dispose(FSourcePtr);
  inherited;
end;
...
constructor TFoundTiffDirectory.Create(const AParser: ITiffParser; const AParent: ITiffDirectory; AIndex: Integer; const AOffset: Int64; const AInternalOffset: Int64 = 0);
begin
  inherited Create;
  FTags := LoadTiffDirectory(AParser.Stream, AParser.Endianness, AParser.BasePosition, AOffset, AInternalOffset, FLoadErrors);
  FIndex := AIndex;
  FParser := AParser;
  FParent := AParent;
end;

constructor TFoundTiffDirectory.Create(const AParser: ITiffParser; const AParent: ITiffDirectory; AIndex: Integer; const Tags: TTiffTagInfoDynArray; const LoadErrors: TMetadataLoadErrors);
begin
  inherited Create;
...
class function TFoundTiffDirectory.TryCreate(const AParser: ITiffParser; const AParent: ITiffDirectory; AIndex: Integer; const AOffset: Int64; out Instance: IFoundTiffDirectory): Boolean;
var
  Tags: TTiffTagInfoDynArray;
  LoadErrors: TMetadataLoadErrors;
begin
  Tags := LoadTiffDirectory(AParser.Stream, AParser.Endianness, AParser.BasePosition, AOffset, 0, LoadErrors);
  Result := (LoadErrors = []) or (Length(Tags) > 2);
  if Result then
    Instance := TFoundTiffDirectory.Create(AParser, AParent, AIndex, Tags, LoadErrors);
end;
...
begin
  ...
  if Result then
    ParsedTag := TFoundTiffTag.Create(Self, Index);
end;

function TFoundTiffDirectory.IsExifThumbailDirectory: Boolean;
begin
  Result := (FIndex = 1) and IsLastDirectoryInFile and TagExists(ttThumbnailOffset, TTiffTag.OffsetDataTypes, 1, 1) and not TagExists(ttStripOffsets) and not TagExists(ttTileOffsets);
end;
...
function TFoundTiffDirectory.TagExists(TagID: TTiffTagID; ValidDataTypes: TTiffDataTypes; MinElementCount, MaxElementCount: LongInt): Boolean;
var
  Index: Integer;
begin
  Result := FindTag(TagID, Index) and (FTags[Index].DataType in ValidDataTypes) and (FTags[Index].ElementCount >= MinElementCount) and (FTags[Index].ElementCount <= MaxElementCount);
end;
...
var
  Index: Integer;
begin
  Result := FindTag(ttThumbnailOffset, Index) and SeekToExifThumbnail(FParser, FTags[Index]);
  if Result then
...
begin
  Result := FindTag(ttThumbnailOffset, Index) and SeekToExifThumbnail(FParser, FTags[Index]);
  if Result then
    Result := TryLoadJpegImageFromStream(Dest, FParser.Stream);
end;

{ TTiffParserEnumerator }

constructor TTiffParserEnumerator.Create(const ASource: ITiffParser; const AMaxPossibleOffset: Int64; const ANextOffset: LongInt);
begin
  FSource := ASource;
...
begin
  ...
  NewDir.FMoreToFollow := FSource.Stream.ReadLongInt(FSource.Endianness, FNextOffset) and (FNextOffset <> 0) and (FNextOffset <= FMaxPossibleOffset);
...
constructor TTiffParser.Create(AStream: TStream; const ABasePosition: Int64; AEndianness: TEndianness);
begin
  inherited Create;
...
function PossibleSubIFDOffset(ID: TTiffTagID; DataType: TTiffDataType; ElementCount: LongInt): Boolean;
begin
  Result := False;
  if ElementCount = 1 then
    case DataType of
      ...
      tdLongWord:
        case ID of
          ttImageWidth, ttImageHeight, ttStripOffsets, ttStripByteCounts, ttTileOffsets, ttTileByteCounts, ttThumbnailOffset, ttThumbnailSize, ttExifImageWidth, ttExifImageHeight: { nope }
            ;
        else
          Result := True;
        end;
...
constructor TTiffDirectoryRewriter.TTagToWrite.Create(AParent: TTiffDirectoryRewriter; const AInfo: TTiffTagInfo);
begin
  inherited Create;
  FInfo := AInfo;
  FParent := AParent;
...
constructor TTiffDirectoryRewriter.TTagToWrite.Create(AParent: TTiffDirectoryRewriter; AID: TTiffTagID; ADataType: TTiffDataType; AElementCount: LongInt; AOriginalDataOffset: LongWord = 0);
begin
  inherited Create;
...
constructor TInternaTiffTagToWrite.Create(AParent: TTiffDirectoryRewriter; const AParser: ITiffParser; const ATagInfo: TTiffTagInfo);
  ...
  begin // the byte counts tag should always come after the offsets one
    if AParent.HasImageData then
      Exit;
    if not AParent.FindTagToWrite(OffsetID, OffsetTag) or not(OffsetTag is TInternaTiffTagToWrite) then
      Exit;
    if (OffsetTag.ElementCount <> ATagInfo.ElementCount) or not(OffsetTag.DataType in TTiffTag.OffsetDataTypes) or not(ATagInfo.DataType in [tdWord, tdLongWord, tdLongInt]) then
      Exit;
    ...
    for Item in Offsets do
      if (Item < 8) or (Item >= BadOffset) then
        Exit;
...
begin
  ...
  case ID of
    ttStripByteCounts:
      LoadImageInfo(ttStripOffsets);
    ttTileByteCounts:
      LoadImageInfo(ttTileOffsets);
    ttThumbnailSize:
      LoadImageInfo(ttThumbnailOffset);
  end;
end;
...
constructor TExternalTiffTagToWrite.Create(AOwner: TTiffDirectoryRewriter; const ATag: ITiffTag);
begin
  inherited Create(AOwner, ATag.ID, ATag.DataType, ATag.ElementCount, ATag.OldDataOffset);
  FTag := ATag;
end;

constructor TExternalTiffTagToWrite.Create(AOwner: TTiffDirectoryRewriter; AID: TTiffTagID; ADataType: TTiffDataType; const ADataSource: IStreamPersist);
begin
...
constructor TTiffSubDirToWrite.Create(AOwner: TTiffDirectoryRewriter; AOffsetID: TTiffTagID; ADirectory: TTiffDirectoryRewriter);
begin
...
begin
  ...
  if Callback <> nil then
    Callback.AddNewTags(Self);
  FDoneInitialization := True;
  if not FRewriteSource or (Source = nil) then
    Exit;
...
procedure TTiffDirectoryRewriter.AddTag(ID: TTiffTagID; const DataSource: IStreamPersist; DataType: TTiffDataType = tdUndefined);
begin
  AddTag(TExternalTiffTagToWrite.Create(Self, ID, DataType, DataSource));
end;

procedure TTiffDirectoryRewriter.AddSubDirectory(OffsetID: TTiffTagID; const SubDirectory: ITiffDirectory; const Callback: ITiffRewriteCallback = nil);
var
...
begin
  Result := False;
  if not FindTagToWrite(ttExifOffset, Tag) or not(Tag is TTiffSubDirToWrite) then
    Exit;
  SubDir := TTiffSubDirToWrite(Tag).Directory;
  if not SubDir.FindTagToWrite(ttMakerNote, Tag) or (Tag.DataType <> tdUndefined) or (Tag.ElementCount <= 4) or (Tag.OldDataOffset < 8) then
    Exit;
  ...
  begin // if an MS application such as the
    ...
    begin
      ...
      if not InRange(RevisedOffset, 8, $FFFFFFF0) then
        Exit;
...
procedure TTiffDirectoryRewriter.InitializeNewOffsets(var NextOffset: LongWord; ProtectedTag: TTagToWrite);
var
...
var
  ...
  Tag: TTagToWrite;
begin
  ProtectedTagToCome := (ProtectedTag <> nil) and (NextOffset < ProtectedTag.NewDataOffset + LongWord(ProtectedTag.ElementCount));
  ...
  begin
    ...
    if Tag = ProtectedTag then
      Continue; // assume its NewDataOffset property has already been set
...
procedure TTiffDirectoryRewriter.Write(AStream: TStream; const ABasePosition: Int64; AEndianness: TEndianness; ANextIFDOffset: LongWord);
var
  Buffer: TBytes;
  I: Integer;
  Tag: TTagToWrite;
begin
  ...
  for I := 0 to High(FImageInfo) do
  begin
    if FImageInfo[I].ByteCount <= 0 then
      Continue;
...
constructor TSimpleTiffRewriteCallbackImpl.Create(const AOwner: IStreamPersistEx; AMetadataTagID: TTiffTagID);
begin
...
procedure TSimpleTiffRewriteCallbackImpl.RewritingOldTag(const Source: ITiffDirectory; TagID: TTiffTagID; DataType: TTiffDataType; var Rewrite: Boolean);
begin
  ...
  try
    ...
    begin
      ...
      for I := 0 to Rewriters.Count - 2 do
        GetRewriter(I).Write(OutStream, NewBasePosition, NewEndianness, GetRewriter(I + 1).NewIFDOffset);
