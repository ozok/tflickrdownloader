...
{$Z2}
  TWindowsStarRating = (urUndefined, urOneStar, urTwoStars, urThreeStars, urFourStars, urFiveStars);
{$Z1}
  ...
  public
    constructor Create(const Section: TExifSection; const ID: TExifTagID; DataType: TExifDataType; ElementCount: LongInt); overload;
    destructor Destroy; override;
...
{$ENDIF}
  ...
  TExifSectionKindEx = (esUserDefined, esGeneral, esDetails, esInterop, esGPS, esThumbnail, esMakerNote);
  ...
  public
    ...
    function TagExists(ID: TExifTagID; ValidDataTypes: TExifDataTypes = [Low(TExifDataType) .. High(TExifDataType)]; MinElementCount: LongInt = 1; MaxElementCount: LongInt = MaxLongInt): Boolean;
...
{$Z2}
  ...
  TTiffOrientation = (toUndefined, toTopLeft, toTopRight, toBottomRight, toBottomLeft, toLeftTop { i.e., rotated } , toRightTop, toRightBottom, toLeftBottom);
  ...
  // esMonochrome was esUndefined before 0.9.7
...
{$Z1}
  ...
  protected
    procedure GetTagInfo(var Section: TExifSectionKind; var XTag, YTag, UnitTag: TExifTagID; var Schema: TXMPNamespace; var XName, YName, UnitName: UnicodeString); virtual; abstract;
    property Owner: TCustomExifData read FOwner;
  public
    constructor Create(AOwner: TCustomExifData);
    procedure Assign(Source: TPersistent); override;
...
{$ENDIF}
  ...
  protected
    procedure GetTagInfo(var Section: TExifSectionKind; var XTag, YTag, UnitTag: TExifTagID; var Schema: TXMPNamespace; var XName, YName, UnitName: UnicodeString); override;
  end;
...
{$Z1}
  ...
  protected
    procedure Assign(const ADegrees, AMinutes, ASeconds: TExifFraction; ADirectionChar: AnsiChar); reintroduce; overload;
    property Owner: TCustomExifData read FOwner;
...
{$ENDIF}
  ...
  public
    procedure Assign(const ADegrees, AMinutes, ASeconds: TExifFraction; ADirection: TGPSLatitudeRef); reintroduce; overload;
    procedure Assign(ADegrees, AMinutes: LongWord; const ASeconds: TExifFraction; ADirection: TGPSLatitudeRef); reintroduce; overload; inline;
    procedure Assign(ADegrees, AMinutes: LongWord; const ASeconds: Currency; ADirection: TGPSLatitudeRef); reintroduce; overload; inline;
    procedure Assign(ADegrees, AMinutes, ASeconds: LongWord; ADirection: TGPSLatitudeRef); reintroduce; overload; inline;
  ...
  public
    procedure Assign(const ADegrees, AMinutes, ASeconds: TExifFraction; ADirection: TGPSLongitudeRef); reintroduce; overload;
    procedure Assign(ADegrees, AMinutes: LongWord; const ASeconds: TExifFraction; ADirection: TGPSLongitudeRef); reintroduce; overload; inline;
    procedure Assign(ADegrees, AMinutes: LongWord; const ASeconds: Currency; ADirection: TGPSLongitudeRef); reintroduce; overload; inline;
    procedure Assign(ADegrees, AMinutes, ASeconds: LongWord; ADirection: TGPSLongitudeRef); reintroduce; overload; inline;
  ...
  TExifDataOffsetsType = (doFromExifStart, doFromMakerNoteStart, doFromIFDStart, doCustomFormat); // !!!added doCustomFormat v1.5.0
  ...
  strict private
    ...
    FTags: TExifSection;
  protected
    class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; overload; virtual; abstract;
    procedure GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType); virtual;
  ...
  const
    BigEndianHeader: array [0 .. HeaderSize - 1] of AnsiChar = 'KDK INFO';
    class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; override;
    procedure GetIFDInfo(SourceTag: TExifTag; var Endianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType); override;
  end
experimental;

TPanasonicMakerNote = class(TExifMakerNote)protected

const
  Header: array [0 .. 11] of AnsiChar = 'Panasonic';
class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; override;
procedure GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType); override;
end;
...
const
  Header: array [0 .. 3] of AnsiChar = 'AOC'#0;

class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; override;
end;

TNikonType1MakerNote = class(TExifMakerNote)protected

const
  Header: array [0 .. 7] of AnsiChar = 'Nikon'#0#1#0;

class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; override;
end;

TNikonType2MakerNote = class(THeaderlessMakerNote)protected class
function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
override;
end;

TNikonType3MakerNote = class(TExifMakerNote)protected

const
  HeaderStart: array [0 .. 6] of AnsiChar = 'Nikon'#0#2;
...
class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; override;
  ...
  public
    ...
    TSonyMakerNote = class(TExifMakerNote)protected
  const
    Header: array [0 .. 7] of AnsiChar = 'SONY DSC';
    class function FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean; override;
      procedure GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType); override;
      end;
  ...
  TCustomExifData = class(TComponent)public type TEnumerator = record strict private FClient: TCustomExifData;
FDoneFirst:
  Boolean;
FSection:
  TExifSectionKind;
    ...
    protected const
      MaxThumbnailSize = $F000;
      class
      function SectionClass: TExifSectionClass;
      virtual;
      procedure AddFromStream(Stream: TStream; TiffImageSource: Boolean = False);
      procedure Changed(Section: TExifSection);
      virtual;
      function GetEmpty: Boolean;
    ...
    private
      class var FMakerNoteClasses: TList;
    public
      class
      procedure RegisterMakerNoteType(AClass: TExifMakerNoteClass; Priority: TMakerNoteTypePriority = mtTestForFirst);
      class
      procedure RegisterMakerNoteTypes(const AClasses: array of TExifMakerNoteClass; Priority: TMakerNoteTypePriority = mtTestForFirst);
      class
      procedure UnregisterMakerNoteType(AClass: TExifMakerNoteClass);
    public
      ...
      procedure EndUpdate;
      procedure GetKeywords(Dest: TStrings);
      overload;
      procedure SetKeywords(const NewWords: array of UnicodeString);
      overload;
      procedure SetKeywords(NewWords: TStrings);
      overload;
      ...
      property Sections[Section: TExifSectionKind]: TExifSection read GetSection;
      default;
    ...
    published
      ...
      property ShutterSpeedValue: TExifSignedFraction index ttShutterSpeedValue read GetDetailsSFraction write SetDetailsSFraction stored False;
      // in APEX; for display, you may well prefer to use ShutterSpeedInMSecs
      ...
      property ThumbnailResolution: TCustomExifResolution read FThumbnailResolution write SetThumbnailResolution stored False;
      end;
      ...
      TExifDataPatcher = class(TCustomExifData) // only supports patching the Exif data in JPEG files
        strict private FOriginalEndianness: TEndianness;
...
{$IFEND}
    ...
    published
      ...
      TExifData = class(TCustomExifData, IStreamPersist, IStreamPersistEx, ITiffRewriteCallback)strict private FRemovePaddingTagsOnSave: Boolean;
      procedure GetGraphicSaveMethod(Stream: TStream; var Method: TGraphicSaveMethod);
      function GetSection(Section: TExifSectionKind): TExtendableExifSection;
      inline;
    protected
...
{$ENDIF}
      ...
      overload;
      inline;
      function LoadFromGraphic(const Graphic: IStreamPersist): Boolean;
      overload;
      function LoadFromGraphic(const FileName: string): Boolean;
      overload;
      procedure LoadFromStream(Stream: TStream);
      procedure RemoveMakerNote;
      procedure RemovePaddingTags;
      procedure SaveToGraphic(const FileName: string);
      overload;
      procedure SaveToGraphic(const Graphic: IStreamPersist);
      overload;
      procedure SaveToStream(Stream: TStream);
      property Sections[Section: TExifSectionKind]: TExtendableExifSection read GetSection;
      default;
{$IF DECLARED(TJPEGData)}
    public // deprecated methods - to be removed in a future release
      ...
      overload;
      deprecated {$IFDEF DepCom}'Use LoadFromGraphic'{$ENDIF};
      procedure LoadFromJPEG(JPEGImage: TJPEGImage);
      overload;
      inline;
      deprecated {$IFDEF DepCom}'Use LoadFromGraphic'{$ENDIF};
      procedure LoadFromJPEG(const FileName: string);
      overload;
      inline;
      deprecated {$IFDEF DepCom}'Use LoadFromGraphic'{$ENDIF};
      procedure SaveToJPEG(const JPEGFileName: string; Dummy: Boolean = True);
      overload;
      inline;
      deprecated {$IFDEF DepCom}'Use SaveToGraphic'{$ENDIF};
      procedure SaveToJPEG(JPEGImage: TJPEGImage);
      overload;
      inline;
      deprecated {$IFDEF DepCom}'Use SaveToGraphic'{$ENDIF};
{$ENDIF}
...
{$IFDEF VCL}
      TJPEGImageEx = class(TJPEGImage)public type TAssignOptions = set of (jaPreserveMetadata);
    strict private
    ...
    public
      ...
      reintroduce;
      overload;
      procedure CreateThumbnail(ThumbnailWidth, ThumbnailHeight: Integer);
      overload;
      ...
      procedure SaveToStream(Stream: TStream);
      override;
      function RemoveMetadata(Kinds: TJPEGMetadataKinds): TJPEGMetadataKinds;
      inline;
      function RemoveSegments(Markers: TJPEGMarkers): TJPEGMarkers;
      inline;
      function Segments(MarkersToLookFor: TJPEGMarkers = TJPEGSegment.AllMarkers): IJPEGHeaderParser;
      inline;
...
begin
  if IsGraphicEmpty(AGraphic) then
    Exit;
{$IFDEF VCL}
  ADest.StretchDraw(ARect, AGraphic);
{$ELSE}
  ADest.DrawBitmap(AGraphic, RectF(0, 0, AGraphic.Width, AGraphic.Height), RectF(ARect.Left, ARect.Top, ARect.Right, ARect.Bottom), 1);
{$ENDIF}
end;
{$ELSE}
...
begin
  case TagID of
    ttGPSVersionID:
      Result := 'GPSVersionID';
    ttGPSLatitude:
      Result := 'GPSLatitude'; // includes ttGPSLatitudeRef
    ttGPSLongitude:
      Result := 'GPSLongitude'; // includes ttGPSLongitudeRef
    ttGPSAltitudeRef:
      Result := 'GPSAltitudeRef';
    ttGPSAltitude:
      Result := 'GPSAltitude';
    ttGPSTimeStamp:
      Result := 'GPSTimeStamp'; // includes GPSDateStamp
    ttGPSSatellites:
      Result := 'GPSSatellites';
    ttGPSStatus:
      Result := 'GPSStatus';
    ttGPSMeasureMode:
      Result := 'GPSMeasureMode';
    ttGPSDOP:
      Result := 'GPSDOP';
    ttGPSSpeedRef:
      Result := 'GPSSpeedRef';
    ...
    ttGPSTrack:
      Result := 'GPSTrack';
    ttGPSImgDirectionRef:
      Result := 'GPSImgDirectionRef';
    ttGPSImgDirection:
      Result := 'GPSImgDirection';
    ttGPSMapDatum:
      Result := 'GPSMapDatum';
    ttGPSDestBearingRef:
      Result := 'GPSDestBearingRef';
    ttGPSDestBearing:
      Result := 'GPSDestBearing';
    ttGPSDestDistance:
      Result := 'GPSDestDistance';
    ttGPSDestLatitude:
      Result := 'GPSDestLatitude'; // includes ttGPSDestLatitudeRef
    ttGPSDestLongitude:
      Result := 'GPSDestLongitude'; // includes ttGPSDestLongitudeRef
    ttGPSDifferential:
      Result := 'GPSDifferential';
  else
    Result := '';
  end;
end;
...
procedure CreateExifThumbnail(Source: TGraphic; Dest: TJPEGImage; MaxWidth: Integer = StandardExifThumbnailWidth; MaxHeight: Integer = StandardExifThumbnailHeight);
var
  Bitmap: TBitmap;
  R: TRect;
begin
...
begin
  ...
  if mkIPTC in KindsToRemove then
    Include(MarkersToLookFor, jmApp13);
        ...
        for Block in Segment do
          ...
          begin
            if SavedBlocks = nil then
              SavedBlocks := TInterfaceList.Create;
...
begin
  Result := Stream.TryReadHeader(TJPEGSegment.ExifHeader, SizeOf(TJPEGSegment.ExifHeader), not MovePosOnSuccess);
end;
...
constructor TExifTag.Create(Section: TExifSection; const Directory: IFoundTiffDirectory; Index: Integer);
var
  Info: TTiffTagInfo;
begin
  ...
  if Info.ElementCount > 0 then
    Directory.Parser.LoadTagData(Info, FData^);
end;

destructor TExifTag.Destroy;
begin
  if Section <> nil then
    Section.TagDeleting(Self);
  if FData <> nil then
    FreeMem(FData);
  FDataStream.Free;
  inherited;
...
procedure TExifTag.Changing(NewID: TExifTagID; NewDataType: TExifDataType; NewElementCount: LongInt; NewData: Boolean);
begin
  if Section <> nil then
...
begin
  if ChangeType in [tcData, tcDataSize] then
    FAsStringCache := '';
  if Section <> nil then
    Section.TagChanged(Self, ChangeType);
end;
...
begin
  case DataType of
    tdAscii:
      Result := string(AnsiString(SeekPtr^));
    tdByte:
      Result := IntToStr(PByte(SeekPtr)^);
    tdWord:
      Result := IntToStr(PWord(SeekPtr)^);
    tdLongWord:
      Result := IntToStr(PLongWord(SeekPtr)^);
    tdShortInt:
      Result := IntToStr(PShortInt(SeekPtr)^);
    tdSmallInt:
      Result := IntToStr(PSmallInt(SeekPtr)^);
    tdLongInt, tdSubDirectory:
      Result := IntToStr(PLongInt(SeekPtr)^);
    tdSingle:
      Result := FloatToStr(PSingle(SeekPtr)^);
    tdDouble:
      Result := FloatToStr(PDouble(SeekPtr)^);
    tdLongWordFraction, tdLongIntFraction:
      Result := PExifFraction(SeekPtr).AsString;
  end;
...
begin
  if (FAsStringCache = '') and (ElementCount <> 0) then
    case DataType of
      ...
      tdUndefined:
        FAsStringCache := BinToHexStr(FData, DataSize);
    else
...
{$RANGECHECKS ON}
            case DataType of
              tdByte:
                PByte(SeekPtr)^ := StrToInt(S);
              tdWord:
                PWord(SeekPtr)^ := StrToInt(S);
              tdLongWord:
                PLongWord(SeekPtr)^ := StrToInt64(S);
              tdShortInt:
                PShortInt(SeekPtr)^ := StrToInt(S);
              tdSmallInt:
                PSmallInt(SeekPtr)^ := StrToInt(S);
              tdLongInt, tdSubDirectory:
                PLongInt(SeekPtr)^ := StrToInt(S);
              tdLongWordFraction, tdLongIntFraction:
                PExifFraction(SeekPtr)^ := TExifFraction.CreateFromString(S);
              tdSingle:
                PSingle(SeekPtr)^ := StrToFloat(S);
              tdDouble:
                PDouble(SeekPtr)^ := StrToFloat(S);
            end;
...
begin
  ...
  if (DataType = tdByte) and (Section <> nil) and (Section.Kind = esGeneral) then
    case ID of
      ttWindowsTitle, ttWindowsComments, ttWindowsAuthor, ttWindowsKeywords, ttWindowsSubject:
        Result := True;
    end;
end;
...
begin
  Result := (ID = ttWindowsPadding) and (DataType = tdUndefined) and (ElementCount >= 2) and (PWord(Data)^ = ttWindowsPadding);
end;

function TExifTag.ReadFraction(Index: Integer; const Default: TExifFraction): TExifFraction;
begin
  if (DataType in [tdLongWordFraction, tdLongIntFraction]) and (Index >= 0) and (Index < ElementCount) then
...
begin
  if (Index < 0) or (Index >= ElementCount) then
    ...
    case DataType of
      tdByte, tdShortInt:
        Result := PByteArray(FData)[Index];
      tdWord, tdSmallInt:
        Result := PWordArray(FData)[Index];
      tdLongWord, tdLongInt:
        Result := PLongWordArray(FData)[Index];
    else
      Result := Default;
    end;
end;
...
procedure TExifTag.UpdateData(NewDataType: TExifDataType; NewElementCount: Integer; const NewData);
const
...
begin
  ...
  if (@NewData = nil) and (NewDataSize <> OldDataSize) and (DataType in IntDataTypes) and (NewDataType in IntDataTypes) and (ElementCount <> 0) and (NewElementCount <> 0) then
  begin
    SetLength(OldIntVals, FElementCount);
    for I := 0 to Min(FElementCount, NewElementCount) - 1 do
      Move(PByteArray(FData)[I * TiffElementSizes[DataType]], OldIntVals[I], TiffElementSizes[DataType]);
  end;
  ...
  begin
    ...
    if OldIntVals <> nil then
      for I := 0 to High(OldIntVals) do
        Move(OldIntVals[I], PByteArray(FData)[I * TiffElementSizes[DataType]], TiffElementSizes[DataType]);
  end;
...
procedure TExifTag.WriteHeader(Stream: TStream; Endianness: TEndianness; DataOffset: LongInt);
var
  I: Integer;
begin
  ...
  begin
    case TiffElementSizes[DataType] of
      1:
        Stream.WriteBuffer(Data^, ElementCount);
      2:
        for I := 0 to ElementCount - 1 do
          Stream.WriteWord(PWordArray(Data)[I], Endianness);
      4:
        Stream.WriteLongWord(PLongWord(Data)^, Endianness);
    end;
...
begin
  ...
  Result := (not CheckID or (ID = ttIPTC)) and TAdobeResBlock.TryReadIPTCHeader(FDataStream, Header, True);
end;
...
function TExifSection.Add(ID: TExifTagID; DataType: TExifDataType; ElementCount: Integer): TExifTag;
var
  I: Integer;
  Tag: TExifTag;
begin
...
begin
  FModified := True;
  if (FOwner <> nil) and (FKind <> esUserDefined) then
    FOwner.Changed(Self);
end;
...
function TExifSection.ForceSetElement(ID: TExifTagID; DataType: TExifDataType; Index: Integer; const Value): TExifTag;
var
  Dest: PByte;
  ...
  if Owner <> nil then
    case Kind of
      esGeneral:
        case OffsetTagID of
          ttExifOffset:
            Result := Owner[esDetails];
          ttGPSOffset:
            Result := Owner[esGPS];
        end;
      esDetails:
        if OffsetTagID = ttInteropOffset then
          Result := Owner[esInterop];
    end;
  if Result = nil then
...
begin
  if Owner = nil then
    ...
    case FKind of
      esDetails, esGPS:
        Result := Owner[esGeneral];
      esInterop, esMakerNote:
        Result := Owner[esDetails];
    else
      Result := nil;
    end;
...
function TExifSection.GetByteValue(TagID: TExifTagID; Index: Integer; Default: Byte; MinValue: Byte = 0; MaxValue: Byte = High(Byte)): Byte;
begin
...
begin
  ...
  if (Owner <> nil) and (SubSecsID <> 0) and Owner[esDetails].Find(SubSecsID, SubSecsTag) and (SubSecsTag.ElementCount > 1) and (SubSecsTag.DataType = tdAscii) then
  begin
    SetLength(S, 3);
...
function TExifSection.GetFractionValue(TagID: TExifTagID; Index: Integer; const Default: TExifFraction): TExifFraction;
var
  Tag: TExifTag;
begin
  if Find(TagID, Tag) and (Tag.DataType in [tdLongWordFraction, tdLongIntFraction]) and (Tag.ElementCount > Index) and (Index >= 0) then
...
function TExifSection.GetLongIntValue(TagID: TExifTagID; Index: Integer; Default: LongInt): LongInt;
begin
  if not TryGetLongWordValue(TagID, Index, Result) then
    Result := Default;
end;
...
function TExifSection.GetLongWordValue(TagID: TExifTagID; Index: Integer; Default: LongWord): LongWord;
begin
  if not TryGetLongWordValue(TagID, Index, Result) then
    Result := Default;
end;

function TExifSection.GetSmallIntValue(TagID: TExifTagID; Index: Integer; Default: SmallInt; MinValue: SmallInt = Low(SmallInt); MaxValue: SmallInt = High(SmallInt)): SmallInt;
begin
  if not TryGetWordValue(TagID, Index, Result) or (Result < MinValue) or (Result > MaxValue) then
    Result := Default;
end;

function TExifSection.GetStringValue(TagID: TExifTagID; const Default: string): string;
begin
  if not TryGetStringValue(TagID, Result) then
    Result := Default;
end;

function TExifSection.GetWindowsStringValue(TagID: TExifTagID; const Default: UnicodeString): UnicodeString;
begin
...
function TExifSection.GetWordValue(TagID: TExifTagID; Index: Integer; Default: Word; MinValue: Word = 0; MaxValue: Word = High(Word)): Word;
begin
  if not TryGetWordValue(TagID, Index, Result) or (Result < MinValue) or (Result > MaxValue) then
    Result := Default;
end;
...
procedure TExifSection.Load(const Directory: IFoundTiffDirectory; TiffImageSource: Boolean);
var
  I: Integer;
  NewTag: TExifTag;
begin
...
procedure TExifSection.SetDateTimeValue(MainID, SubSecsID: TExifTagID; const Value: TDateTimeTagValue);
var
  SubSecsTag: TExifTag;
begin
  if (Owner = nil) or (SubSecsID = 0) then
    SubSecsTag := nil
  else if not Owner[esDetails].Find(SubSecsID, SubSecsTag) then
...
procedure TExifSection.DoSetFractionValue(TagID: TExifTagID; Index: Integer; DataType: TExifDataType; const Value);
var
  Tag: TExifTag;
begin
...
procedure TExifSection.SetFractionValue(TagID: TExifTagID; Index: Integer; const Value: TExifFraction);
begin
...
procedure TExifSection.SetSignedFractionValue(TagID: TExifTagID; Index: Integer; const Value: TExifSignedFraction);
begin
  DoSetFractionValue(TagID, Index, tdExifFraction, Value);
end;
...
procedure TExifSection.TagChanging(Tag: TExifTag; NewID: TExifTagID; NewDataType: TExifDataType; NewElementCount: LongInt; NewData: Boolean);
var
  NewDataSize: Integer;
  OtherTag: TExifTag;
begin
...
function TExifSection.TagExists(ID: TExifTagID; ValidDataTypes: TExifDataTypes; MinElementCount, MaxElementCount: LongInt): Boolean;
var
  Tag: TExifTag;
begin
  Result := Find(ID, Tag) and (Tag.DataType in ValidDataTypes) and (Tag.ElementCount >= MinElementCount) and (Tag.ElementCount <= MaxElementCount);
end;
...
var
  Tag: TExifTag;
begin
  Result := Find(TagID, Tag) and (Tag.DataType in [tdByte, tdShortInt, tdUndefined]) and (Tag.ElementCount > Index) and (Index >= 0);
  if Result then
    Byte(Value) := PByteArray(Tag.Data)[Index];
end;

function TExifSection.TryGetLongWordValue(TagID: TExifTagID; Index: Integer; var Value): Boolean;
var
  Tag: TExifTag;
begin
  ...
  if Result then
    case Tag.DataType of
      tdByte, tdShortInt:
        LongWord(Value) := PByteArray(Tag.Data)[Index];
      tdWord, tdSmallInt:
        LongWord(Value) := PWordArray(Tag.Data)[Index];
      tdLongWord, tdLongInt:
        LongWord(Value) := PLongWordArray(Tag.Data)[Index];
    else
      Result := False;
    end;
end;
...
function TExifSection.TryGetWordValue(TagID: TExifTagID; Index: Integer; var Value): Boolean;
var
  Tag: TExifTag;
begin
  Result := Find(TagID, Tag) and (Index < Tag.ElementCount) and (Index >= 0);
  if Result then
    case Tag.DataType of
      tdByte, tdShortInt:
        Word(Value) := PByteArray(Tag.Data)[Index];
      tdWord, tdSmallInt:
        Word(Value) := PWordArray(Tag.Data)[Index];
    else
      Result := False;
    end;
end;
...
function TExtendableExifSection.Add(ID: TExifTagID; DataType: TExifDataType; ElementCount: LongInt): TExifTag;
begin
  Result := inherited Add(ID, DataType, ElementCount);
end;

function TExtendableExifSection.AddOrUpdate(ID: TExifTagID; DataType: TExifDataType; ElementCount: Integer): TExifTag;
begin
...
function TExtendableExifSection.AddOrUpdate(ID: TExifTagID; DataType: TExifDataType; ElementCount: Integer; const Data): TExifTag;
begin
  if not Find(ID, Result) then
    Result := Add(ID, DataType, ElementCount);
...
function TExtendableExifSection.AddOrUpdate(ID: TExifTagID; DataType: TExifDataType; const Source: IStreamPersist): TExifTag;
var
...
begin
  ...
  try
    ...
    Result := AddOrUpdate(ID, DataType, Ceil(Stream.Size / TiffElementSizes[DataType]), Stream.Memory^);
  finally
    Stream.Free;
  end;
end;
...
begin
  with FOwner[esDetails] do
    Result := not TagExists(ttFlash, [tdWord, tdSmallInt]) and not TagExists(ttFlashEnergy, [tdLongWordFraction, tdLongIntFraction]);
end;
...
begin
  Result := not Section.TagExists(FXTagID, [tdLongWordFraction, tdLongWordFraction]) or not Section.TagExists(FYTagID, [tdLongWordFraction, tdLongWordFraction]);
end;
...
procedure TImageResolution.GetTagInfo(var Section: TExifSectionKind; var XTag, YTag, UnitTag: TExifTagID; var Schema: TXMPNamespace; var XName, YName, UnitName: UnicodeString);
begin
  Section := esGeneral;
  Schema := xsTIFF;
  XName := 'XResolution';
...
procedure TFocalPlaneResolution.GetTagInfo(var Section: TExifSectionKind; var XTag, YTag, UnitTag: TExifTagID; var Schema: TXMPNamespace; var XName, YName, UnitName: UnicodeString);
begin
  Section := esDetails;
...
procedure TThumbnailResolution.GetTagInfo(var Section: TExifSectionKind; var XTag, YTag, UnitTag: TExifTagID; var Schema: TXMPNamespace; var XName, YName, UnitName: UnicodeString);
begin
  Section := esThumbnail;
end;
...
begin
  ...
  case FOwner.XMPWritePolicy of
    xwRemove:
      FOwner.XMPPacket.RemoveProperty(XMPSchema, XMPName);
    xwAlwaysUpdate:
      if FOwner.XMPPacket.FindSchema(XMPSchema, Schema) and Schema.FindProperty(XMPName, Prop) then
        WriteXMP;
  else
    WriteXMP;
  end
...
procedure TGPSCoordinate.Assign(const ADegrees, AMinutes, ASeconds: TExifFraction; ADirectionChar: AnsiChar);
var
  NewElemCount: Integer;
  Tag: TExifTag;
begin
  if ASeconds.MissingOrInvalid then
    NewElemCount := 2
  else
    NewElemCount := 3;
  ...
  if NewElemCount > 2 then
    PExifFractionArray(Tag.Data)[2] := ASeconds;
  Tag.Changed;
...
procedure TGPSLatitude.Assign(const ADegrees, AMinutes, ASeconds: TExifFraction; ADirection: TGPSLatitudeRef);
const
...
procedure TGPSLatitude.Assign(ADegrees, AMinutes: LongWord; const ASeconds: TExifFraction; ADirection: TGPSLatitudeRef);
begin
  Assign(TExifFraction.Create(ADegrees), TExifFraction.Create(AMinutes), ASeconds, ADirection);
end;

procedure TGPSLatitude.Assign(ADegrees, AMinutes: LongWord; const ASeconds: Currency; ADirection: TGPSLatitudeRef);
begin
  Assign(TExifFraction.Create(ADegrees), TExifFraction.Create(AMinutes), TExifFraction.Create(ASeconds), ADirection);
end;

procedure TGPSLatitude.Assign(ADegrees, AMinutes, ASeconds: LongWord; ADirection: TGPSLatitudeRef);
begin
  Assign(TExifFraction.Create(ADegrees), TExifFraction.Create(AMinutes), TExifFraction.Create(ASeconds), ADirection);
end;
...
begin
  case inherited Direction of
    ...
    Result := ltMissingOrInvalid;
  end;
end;

{ TGPSLongitude }

procedure TGPSLongitude.Assign(const ADegrees, AMinutes, ASeconds: TExifFraction; ADirection: TGPSLongitudeRef);
const
...
procedure TGPSLongitude.Assign(ADegrees, AMinutes: LongWord; const ASeconds: TExifFraction; ADirection: TGPSLongitudeRef);
begin
  Assign(TExifFraction.Create(ADegrees), TExifFraction.Create(AMinutes), ASeconds, ADirection);
end;

procedure TGPSLongitude.Assign(ADegrees, AMinutes: LongWord; const ASeconds: Currency; ADirection: TGPSLongitudeRef);
begin
  Assign(TExifFraction.Create(ADegrees), TExifFraction.Create(AMinutes), TExifFraction.Create(ASeconds), ADirection);
end;

procedure TGPSLongitude.Assign(ADegrees, AMinutes, ASeconds: LongWord; ADirection: TGPSLongitudeRef);
begin
  Assign(TExifFraction.Create(ADegrees), TExifFraction.Create(AMinutes), TExifFraction.Create(ASeconds), ADirection);
end;
...
begin
  ...
  begin
    if FSection = High(TExifSectionKind) then
      Exit;
    Inc(FSection);
  end;
  Result := True;
...
class procedure TCustomExifData.RegisterMakerNoteType(AClass: TExifMakerNoteClass; Priority: TMakerNoteTypePriority);
begin
  if AClass = nil then
    Exit;
  ...
  case Priority of
    mtTestForLast:
      FMakerNoteClasses.Insert(0, AClass);
    mtTestForFirst:
      FMakerNoteClasses.Add(AClass);
  end;
end;

class procedure TCustomExifData.RegisterMakerNoteTypes(const AClasses: array of TExifMakerNoteClass; Priority: TMakerNoteTypePriority);
var
      ...
      case Directory.Index of
        0:
          begin
            ...
            if (TiffImageSource or XMPPacket.Empty) and Directory.FindTag(ttXMP, TiffTag) and TiffTag.IsXMPBlock then
...
begin
  if FSections[esDetails].TryGetWordValue(ttColorSpace, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Result of
        csRGB, csAdobeRGB, csWideGamutRGB, csICCProfile, csUncalibrated:
          Exit;
      end;
  Result := csTagMissing;
end;
...
begin
  ...
  if (Result = '') and FSections[esDetails].Find(ttUserComment, Tag) and (Tag.DataType in [tdByte, tdUndefined]) and (Tag.ElementCount > 9) then
  begin
...
begin
  if FSections[esDetails].TryGetWordValue(ttContrast, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifContrast)) .. Ord(High(TExifContrast)):
          Exit;
      end;
  Result := cnTagMissing;
end;
...
begin
  case TagID of
    ttDateTimeOriginal:
      SubSecsID := ttSubsecTimeOriginal;
    ttDateTimeDigitized:
      SubSecsID := ttSubsecTimeDigitized;
  else
    SubSecsID := 0;
  end;
...
begin
  ...
  if not Result.MissingOrInvalid then
    Exit;
  FocalLengthFrac := FocalLength;
  if FocalLengthFrac.MissingOrInvalid then
    Exit;
  ExifWidth := ExifImageWidth;
  ExifHeight := ExifImageHeight;
  if (ExifWidth = 0) or (ExifHeight = 0) then
    Exit;
  ...
  if (CCDWidth = 0) or (CCDHeight = 0) then
    Exit;
...
begin
  if FSections[esDetails].TryGetWordValue(ttExposureMode, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifExposureMode)) .. Ord(High(TExifExposureMode)):
          Exit;
      end;
  Result := exTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttExposureProgram, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifExposureProgram)) .. Ord(High(TExifExposureProgram)):
          Exit;
      end;
  Result := eeTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetByteValue(ttFileSource, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifFileSource)) .. Ord(High(TExifFileSource)):
          Exit;
      end;
  Result := fsUnknown;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttGainControl, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifGainControl)) .. Ord(High(TExifGainControl)):
          Exit;
      end;
  Result := egTagMissing;
end;
...
begin
  if FSections[esGPS].TryGetByteValue(ttGPSAltitudeRef, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TGPSAltitudeRef)) .. Ord(High(TGPSAltitudeRef)):
          Exit;
      end;
  Result := alTagMissing;
end;
...
begin
  S := GPSDateStamp;
  if (Length(S) <> 10) or not TryStrToInt(Copy(S, 1, 4), Year) or not TryStrToInt(Copy(S, 6, 2), Month) or not TryStrToInt(Copy(S, 9, 2), Day) or not TryEncodeDate(Year, Month, Day, DatePart) then
    DatePart := 0;
  Hour := GPSTimeStampHour;
  Minute := GPSTimeStampMinute;
  Second := GPSTimeStampSecond;
  if Hour.MissingOrInvalid or Minute.MissingOrInvalid or Second.MissingOrInvalid or not TryEncodeTime(Trunc(Hour.Quotient), Trunc(Minute.Quotient), Trunc(Second.Quotient), 0, TimePart) then
  begin
    if DatePart = 0 then
      Result := TDateTimeTagValue.CreateMissingOrInvalid
    else
...
begin
  if FSections[esGPS].TryGetWordValue(ttGPSDifferential, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TGPSDifferential)) .. Ord(High(TGPSDifferential)):
          Exit;
      end;
  Result := dfTagMissing;
end;
...
begin
  ...
  if FSections[esGPS].TryGetStringValue(ttGPSSpeedRef, S) and (S <> '') then
    case UpCase(S[1]) of
      'K':
        Result := srKilometresPerHour;
      'M':
        Result := srMilesPerHour;
      'N':
        Result := srKnots;
    end;
end;
...
begin
  ...
  if FSections[esGPS].TryGetStringValue(ttGPSStatus, S) and (S <> '') then
    case UpCase(S[1]) of
      'A':
        Result := stMeasurementActive;
      'V':
        Result := stMeasurementVoid;
    end;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttLightSource, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifLightSource)) .. Ord(High(TExifLightSource)):
          Exit;
      end;
  Result := elTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttMeteringMode, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifMeteringMode)) .. Ord(High(TExifMeteringMode)):
          Exit;
      end;
  Result := emTagMissing;
end;
...
begin
  if FSections[Section].TryGetWordValue(ttOrientation, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifOrientation)) .. Ord(High(TExifOrientation)):
          Exit;
      end;
  Result := toUndefined;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttCustomRendered, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifRendering)) .. Ord(High(TExifRendering)):
          Exit;
      end;
  Result := erTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttSaturation, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifSaturation)) .. Ord(High(TExifSaturation)):
          Exit;
      end;
  Result := euTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttSceneCaptureType, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifSceneCaptureType)) .. Ord(High(TExifSceneCaptureType)):
          Exit;
      end;
  Result := ecTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetByteValue(ttSceneType, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifSceneType)) .. Ord(High(TExifSceneType)):
          Exit;
      end;
  Result := esUnknown;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttSensingMethod, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifSensingMethod)) .. Ord(High(TExifSensingMethod)):
          Exit;
      end;
  Result := esTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttSharpness, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifSharpness)) .. Ord(High(TExifSharpness)):
          Exit;
      end;
  Result := ehTagMissing;
end;
...
begin
  if FSections[esDetails].TryGetWordValue(ttSubjectDistanceRange, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifSubjectDistanceRange)) .. Ord(High(TExifSubjectDistanceRange)):
          Exit;
      end;
  Result := edTagMissing;
end;
...
var
  Tag: TExifTag;
begin
  if FSections[esDetails].Find(ttSubjectLocation, Tag) and (Tag.DataType in [tdWord, tdSmallInt]) and (Tag.ElementCount >= 2) then
...
begin
  ...
  else if TryStrToInt(XMPPacket[xsXMPBasic].Properties['Rating'].ReadValue, I) then
...
begin
  if FSections[esDetails].TryGetWordValue(ttWhiteBalance, 0, Result) then
    if not EnsureEnumsInRange then
      ...
      case Ord(Result) of
        Ord(Low(TExifWhiteBalanceMode)) .. Ord(High(TExifWhiteBalanceMode)):
          Exit;
      end;
  Result := ewTagMissing;
end;
...
begin // we'll be a bit more intelligent though.
  ...
  if not ContainsOnlyASCII(Value) then
    FSections[esGeneral].Remove(ttArtist)
  else if FSections[esGeneral].Find(ttArtist, Tag) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Move(PWideChar(Value)^, PByteArray(Tag.Data)[SizeOf(UCID_Unicode)], NewSize - SizeOf(UCID_Unicode));
      end;
    end;
  end;
end;
...
begin
  ...
  case TagID of
    ttDateTimeOriginal:
      XMPName := 'DateTimeOriginal';
    ttDateTimeDigitized:
      XMPName := 'DateTimeDigitized';
  else
    Exit;
  end;
...
procedure TCustomExifData.SetDetailsFraction(TagID: Integer; const Value: TExifFraction);
var
  XMPName: string;
begin
  ...
  case TagID of
    ttApertureValue:
      XMPName := 'ApertureValue';
    ttCompressedBitsPerPixel:
      XMPName := 'CompressedBitsPerPixel';
    ttDigitalZoomRatio:
      XMPName := 'DigitalZoomRatio';
    ttExposureBiasValue:
      XMPName := 'ExposureBiasValue';
    ttExposureTime:
      XMPName := 'ExposureTime';
    ttFNumber:
      XMPName := 'FNumber';
    ttMaxApertureValue:
      XMPName := 'MaxApertureValue';
    ttSubjectDistance:
      XMPName := 'SubjectDistance';
  else
    Exit;
  end;
...
procedure TCustomExifData.SetDetailsSFraction(TagID: Integer; const Value: TExifSignedFraction);
var
  PropName: string;
begin
  ...
  case TagID of
    ttBrightnessValue:
      PropName := 'BrightnessValue';
    ttExposureBiasValue:
      PropName := 'ExposureBiasValue';
    ttShutterSpeedValue:
      PropName := 'ShutterSpeedValue';
  else
    Exit;
  end;
...
begin
  ...
  case TagID of
    ttImageUniqueID:
      XMPName := 'ImageUniqueID';
    ttRelatedSoundFile:
      XMPName := 'RelatedSoundFile';
    ttSpectralSensitivity:
      XMPName := 'SpectralSensitivity';
  else
    Exit;
  end;
...
procedure TCustomExifData.SetDetailsWordEnum(ID: TExifTagID; const XMPName: UnicodeString; const Value);
begin
...
const
  PropNames: array [ttExifImageWidth .. ttExifImageHeight] of UnicodeString = ('PixelXDimension', 'PixelYDimension');
var
  Tag: TExifTag;
begin
  ...
  Tag := nil;
  if (NewValue <= High(Word)) and FSections[esDetails].Find(ID, Tag) and (Tag.DataType = tdWord) and (Tag.ElementCount = 1) then
...
begin
  ...
  case TagID of
    ttCopyright:
      XMPPacket.UpdateProperty(xsDublinCore, 'rights', xpAltArray, Value);
    ttImageDescription:
      if (Value <> '') or (XMPWritePolicy = xwRemove) or not FSections[esDetails].TagExists(ttWindowsSubject) then
        XMPPacket.UpdateProperty(xsDublinCore, 'description', xpAltArray, Value);
    ttMake:
      XMPPacket.UpdateProperty(xsTIFF, 'Make', Value);
    ttModel:
      XMPPacket.UpdateProperty(xsTIFF, 'Model', Value);
    ttSoftware:
      begin
...
procedure TCustomExifData.SetGeneralWinString(TagID: Integer; const Value: UnicodeString);
begin
  ...
  case TagID of
    ...
    ttWindowsSubject:
      XMPPacket.UpdateProperty(xsDublinCore, 'description', xpAltArray, Value);
    ttWindowsTitle:
      XMPPacket.UpdateProperty(xsDublinCore, 'title', xpAltArray, Value);
  end;
end;
...
procedure TCustomExifData.SetGPSTimeStamp(const Index: Integer; const Value: TExifFraction);
begin
...
begin
  if Value <> FISOSpeedRatings then
    FISOSpeedRatings.Assign(Value);
end;
...
begin
  ...
  if TExifSectionKind(SectionKind) <> esGeneral then
    Exit;
  if Value = toUndefined then
    XMPValue := ''
  else
...
{$ENDIF}
    ...
    begin
      if (FStream = nil) or not Modified then
        Exit;
      ...
      begin
        ...
        try
          ...
          begin
            ...
            FillChar(PAnsiChar(XMPStream.Memory)[XMPStream.Size], FXMPPacketSizeInSource - XMPStream.Size, $20);
          end
          else
          begin
            ...
            begin
              ...
              for Segment in JPEGHeader(Stream) do
                if Segment.MarkerNum <> jmApp1 then
                  Break;
...
{$IF Declared(TGraphic)}
    procedure TExifData.CreateThumbnail(Source: TGraphic; ThumbnailWidth, ThumbnailHeight: Integer);
    begin
...
{$ENDIF}
    ...
    begin
      ...
      for Block in ParsePSDHeader(InStream, Info) do
        if not Block.IsExifBlock and not Block.IsXMPBlock then
          NewBlocks.Add(Block);
    ...
    procedure TExifData.RewritingOldTag(const Source: ITiffDirectory; TagID: TTiffTagID; DataType: TTiffDataType; var Rewrite: Boolean);
    begin
      if IsKnownExifTagInMainIFD(TagID, DataType) then
        Rewrite := False;
    end;
...
{$ENDIF}
  ...
  { TExifData.SaveToStream: sections are written out in TExifSection order, with a
    ...
    var
      ...
      procedure InitSavingInfo(Kind: TExifSectionKind; const OffsettedSections: array of TOffsetSectionKind);
      const
        OffsetTagIDs: array [TOffsetSectionKind] of TExifTagID = (ttExifOffset, ttInteropOffset, ttGPSOffset, ttThumbnailOffset);
      var
      ...
      begin
        for Client in OffsettedSections do
          if (SavingInfo[Client].DirectorySize > 0) or (Client = Kind) { as for thumbnail } then
            OffsetTags[Client] := Sections[Kind].AddOrUpdate(OffsetTagIDs[Client], tdLongWord, 1, PLongWord(nil)^)
          else
            Sections[Kind].Remove(OffsetTagIDs[Client]);
        if (Kind <> esGeneral) and (Sections[Kind].Count = 0) then
          Exit; // don't write out empty sections
        for Tag in Sections[Kind] do
          if Tag.DataSize > 4 then
            Inc(SavingInfo[Kind].OffsettedDataSize, Tag.DataSize);
      ...
      var
        ...
        Tag: TExifTag;
      begin
        if SavingInfo[Kind].DirectorySize = 0 then
          Exit;
    ...
    begin
      if RemovePaddingTagsOnSave then
        RemovePaddingTags;
      ...
      begin
        ...
        PreserveMakerNotePos := Find(ttMakerNote, MakerNoteTag) and (MakerNoteTag.OriginalDataOffset <> 0) and (MakerNoteTag.DataSize > 4); // if size is 4 or less, then data isn't offsetted
...
{$ENDIF}
        ...
        begin
          ...
          if PreserveMakerNotePos and (SavingInfo[Kind].EndOffsetPlus1 > MakerNoteDataOffset) and (SavingInfo[Kind].StartOffset < MakerNoteDataOffset + MakerNoteTag.OriginalDataSize) then
...
{$ENDIF}
  { TExifMakerNote }
    ...
    begin
      inherited Create;
      FTags := ASection;
      if ClassType = TUnrecognizedMakerNote then
        Exit;
      if not ASection.Owner[esDetails].Find(ttMakerNote, SourceTag) or not FormatIsOK(SourceTag, HeaderSize) then
        raise EInvalidMakerNoteFormat.CreateRes(@SInvalidMakerNoteFormat);
      ...
      case FDataOffsetsType of
        doCustomFormat:
          Exit;
        doFromExifStart:
          BasePosition := -SourceTag.OriginalDataOffset;
        doFromIFDStart:
          BasePosition := HeaderSize;
        doFromMakerNoteStart:
          BasePosition := 0;
      else
      ...
      try
        Tags.Load(ParseTiffDirectory(Stream, FEndianness, BasePosition, HeaderSize - BasePosition, InternalOffset), False);
        ...
        if (Tags.Count = 0) or ((Tags.LoadErrors <> []) and not(leBadOffset in Tags.LoadErrors) and (Tags.Count < 3)) then
        begin
    ...
    var
      HeaderSize: Integer;
    begin
      Result := (SourceTag.DataType = tdUndefined) and (SourceTag.ElementCount >= 2) and FormatIsOK(SourceTag, HeaderSize);
    end;

    procedure TExifMakerNote.GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType);
    begin
    end;
  ...
  { TCanonMakerNote }
    ...
    procedure TCanonMakerNote.GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType);
    begin
  ...
  { TKodakMakerNote }
    ...
    class function TKodakMakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := HeaderSize;
      Result := (SourceTag.ElementCount > HeaderSize) and (StrLComp(PAnsiChar(SourceTag.Data), 'KDK', 3) = 0);
    end;

    procedure TKodakMakerNote.GetIFDInfo(SourceTag: TExifTag; var Endianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType);
    var
    ...
    begin
      ...
      for I := Low(TagSpecs) to High(TagSpecs) do
        if SourceTag.DataStream.TryReadBuffer(Buffer, TiffElementSizes[TagSpecs[I].DataType] * TagSpecs[I].ElementCount) then
        begin
  ...
  { TPanasonicMakerNote }
    ...
    class function TPanasonicMakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := SizeOf(Header);
      Result := (SourceTag.ElementCount > HeaderSize) and CompareMem(SourceTag.Data, @Header, HeaderSize);
    end;

    procedure TPanasonicMakerNote.GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType);
    begin
      ProbableEndianness := SmallEndian;
    end;
  ...
  { TPentaxMakerNote }
    ...
    class function TPentaxMakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := SizeOf(Header);
      Result := (SourceTag.ElementCount > HeaderSize) and CompareMem(SourceTag.Data, @Header, HeaderSize);
    end;
  ...
  { TNikonType1MakerNote }
    ...
    class function TNikonType1MakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := SizeOf(Header);
      Result := (SourceTag.ElementCount > HeaderSize) and CompareMem(SourceTag.Data, @Header, HeaderSize);
    end;
  ...
  { TNikonType2MakerNote }
    ...
    class function TNikonType2MakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := 0;
  ...
  { TNikonType3MakerNote }
    ...
    class function TNikonType3MakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := 18;
      Result := (SourceTag.ElementCount > HeaderSize) and CompareMem(SourceTag.Data, @HeaderStart, SizeOf(HeaderStart));
    end;

    procedure TNikonType3MakerNote.GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType);
    var
      SeekPtr: PAnsiChar;
    begin
  ...
  { TSonyMakerNote }
    ...
    class function TSonyMakerNote.FormatIsOK(SourceTag: TExifTag; out HeaderSize: Integer): Boolean;
    begin
      HeaderSize := 12;
      Result := (SourceTag.ElementCount > HeaderSize) and CompareMem(SourceTag.Data, @Header, SizeOf(Header));
    end;

    procedure TSonyMakerNote.GetIFDInfo(SourceTag: TExifTag; var ProbableEndianness: TEndianness; var DataOffsetsType: TExifDataOffsetsType);
    begin
...
{$IFDEF DummyTJpegImage}
    ...
    begin
      ...
      if (JpegSize = 0) and (FData.Size = 0) then
        Exit;
      FWidth := 0;
      FData.Size := JpegSize;
    ...
    var
      ...
      Segment: IFoundJPEGSegment;
    begin
      if (FWidth <> 0) or (FData.Size = 0) then
        Exit;
      FData.Position := 0;
