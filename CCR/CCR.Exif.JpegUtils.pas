...
procedure WriteJPEGSegmentToStream(Stream: TStream; MarkerNum: TJPEGMarker; const Data; DataSize: Word); overload; deprecated
{$IFDEF DepCom}'Use WriteJPEGSegment in CCR.Exif.BaseUtils instead'{$ENDIF};
procedure WriteJPEGSegmentToStream(Stream: TStream; MarkerNum: TJPEGMarker; Data: TStream; DataSize: Word = 0); overload; deprecated
{$IFDEF DepCom}'Use WriteJPEGSegment in CCR.Exif.BaseUtils instead'{$ENDIF};
...
begin
  ...
  if ADataSource <> nil then
    ADataSource.SaveToStream(FData);
end;
...
begin
  ...
  if Len <> 0 then
    Stream.WriteBuffer(FData.Memory^, Len);
  if Odd(Len) then
    Stream.WriteByte(0);
end;
...
begin
  ...
  if MarkerNum in MarkersWithNoData then
    Dec(Result, 2);
end;
...
begin
  Result := JPEGHeader(TFileStream.Create(JPEGFile, fmOpenRead), MarkersToLookFor, soOwned);
end;

function JPEGHeader(Image: TJPEGImage; const MarkersToLookFor: TJPEGMarkers): IJPEGHeaderParser;
var
...
begin
  ...
  if Result then
    Stream.Seek(-SizeOf(JPEGFileHeader), soCurrent);
end;
...
procedure WriteJPEGSegmentToStream(Stream: TStream; MarkerNum: TJPEGMarker; Data: TStream; DataSize: Word); overload;
var
  Buffer: Pointer;
...
function CreateAdobeBlock(ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist): IAdobeBlock; overload;
begin
  Result := TAdobeBlock.Create('8BIM', ATypeID, AName, ADataSource);
end;

function CreateAdobeBlock(const ASignature: AnsiString; ATypeID: Word; const AName: AnsiString; const ADataSource: IStreamPersist): IAdobeBlock; overload;
begin
