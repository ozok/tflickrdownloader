...
type
  ...
  protected
    constructor Create(AKind: TXMPNamespace; const APreferredPrefix, AURI: UnicodeString); overload;
  ...
  public
    constructor Create(ASchema: TXMPSchema; AParentProperty: TXMPProperty; const ASourceNode: IDOMNode = nil);
    ...
    procedure UpdateSubProperty(const SubPropName: UnicodeString; SubPropKind: TXMPPropertyKind; const NewValue: UnicodeString); overload;
  ...
  strict private
    ...
    procedure DoUpdateProperty(Policy: TXMPWritePolicy; SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; PropKind: TXMPPropertyKind; const NewValue: UnicodeString);
    procedure DoUpdateArrayProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; ArrayPropKind: TXMPPropertyKind; const NewValues: array of UnicodeString); overload;
  protected
  ...
  public
    ...
    procedure RemoveProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString); overload;
    procedure RemoveProperties(SchemaKind: TXMPKnownNamespace; const PropNames: array of UnicodeString); overload;
    procedure UpdateProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; PropKind: TXMPPropertyKind; const NewValue: UnicodeString); overload;
    procedure UpdateBagProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValues: array of UnicodeString); overload;
    procedure UpdateBagProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValueList: UnicodeString); overload; inline; // commas and/or semi-colons act as delimiters
    procedure UpdateSeqProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValues: array of UnicodeString); overload;
    procedure UpdateSeqProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValueList: UnicodeString); overload; inline; // commas and/or semi-colons act as delimiters
    procedure UpdateProperty(SchemaKind: TXMPKnownNamespace; const PropName, NewValue: UnicodeString); overload; inline;
    procedure UpdateProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValue: Integer); overload;
    procedure UpdateDateTimeProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValue: TDateTimeTagValue; ApplyLocalBias: Boolean = True); overload;
...
GetTimeZoneInformationForYear:
  ...
  function(wYear: Word; DynInfo: Pointer; var TZInfo: TTimeZoneInformation): BOOL;
  stdcall = nil;
TzSpecificLocalTimeToSystemTime:
    function(var TZInfo: TTimeZoneInformation; const LocalTime: TSystemTime; out UTCTime: TSystemTime): BOOL;
    stdcall = nil;
    var
      DoFallback: Boolean;
...
{$ENDIF}
    ...
    begin
      ...
      if ApplyLocalBias then
        Result := Format('%s%s%.2d:%.2d', [Result, PlusNegSyms[GetUTCOffset(Value, Hours, Mins) >= 0], Hours, Mins]);
    end;
    ...
    begin
      ...
      begin
        if Node.nodeType = ELEMENT_NODE then
          case IndexStr(Node.localName, ['RDF', 'xmpmeta', 'xapmeta']) of
            0:
              Exit; // support ExifTool's XML dumps, which don't parent the RDF node
            1 .. 2:
              Break;
          end;
      ...
      begin
        ...
        while Node <> nil do
        begin
          if (Node.nodeName = 'rdf:RDF') and (Node.nodeType = ELEMENT_NODE) then
            Break;
          Node := Node.nextSibling;
        end;
      end;
  ...
  { TXMPNamespaceInfo }
    ...
    constructor TXMPNamespaceInfo.Create(AKind: TXMPNamespace; const APreferredPrefix, AURI: UnicodeString);
    begin
      inherited Create;
      FKind := AKind;
  ...
  { TXMPProperty }
    ...
    constructor TXMPProperty.Create(ASchema: TXMPSchema; AParentProperty: TXMPProperty; const ASourceNode: IDOMNode);
    ...
    begin
      ...
      if ASourceNode = nil then
        Exit;
      // figure out our kind
      PropAndNodeStructure := Supports(ASourceNode, IDOMElement, SourceAsElem) and (SourceAsElem.getAttributeNS(RDF.URI, 'parseType') = 'Resource');
      ...
      begin
        ...
        begin
          if FKind = xpStructure then
            ...
            DoAdd := (FKind <> xpAltArray) or (ChildNode.attributes.getNamedItem(XMLLangAttrName) <> nil);
        end;
        if DoAdd then
          FSubProperties.Add(TXMPProperty.Create(Schema, Self, ChildNode));
    ...
    begin
      case Kind of
        xpSimple:
          if Pointer(FValue) <> nil then
            Result := FValue
          else
            Result := Default;
        xpAltArray:
          Result := SubProperties[DefaultLangIdent].ReadValue(Default);
        xpBagArray, xpSeqArray:
          case SubPropertyCount of
            0:
              Result := Default;
            1:
              Result := SubProperties[0].ReadValue(Default);
          else
    ...
    begin
      if Value = FParentNamespace then
        Exit;
    ...
    begin
      ...
      if NewCount < FSubProperties.Count then
        ...
        begin
          ...
          if HasNamedSubProperties then
            NewSubProp.FName := Format('SubProp%d', [I]);
    ...
    procedure TXMPProperty.UpdateSubProperty(const SubPropName: UnicodeString; SubPropKind: TXMPPropertyKind; const NewValue: UnicodeString);
    var
      SubProp: TXMPProperty;
    begin
      ...
      begin
        if FSchema.Owner.UpdatePolicy = xwUpdateIfExists then
          Exit;
        if not(Kind in [xpStructure, xpAltArray]) then
          Kind := xpStructure;
    ...
    begin
      case Kind of
        ...
        xpStructure:
          raise EInvalidXMPOperation.CreateRes(@SCannotWriteSingleValueToStructureProperty);
        xpAltArray:
          SubProperties[DefaultLangIdent].WriteValue(NewValue);
      else
        ...
        try
          ...
          if BeginPos <= TotalLen then
            Strings.Add(Copy(NewValue, BeginPos, TotalLen));
  ...
  { TXMPPacket }
    ...
    begin
      FDataToLazyLoad := nil;
      if (FSchemas.Count = 0) and (FAboutAttributeValue = '') then
        Exit;
    ...
    begin
      ...
      if not Empty then
        NewBlocks.Add(CreateAdobeBlock(TAdobeResBlock.XMPTypeID, Self));
      for Block in ParsePSDHeader(InStream, Info) do
        if not Block.IsXMPBlock then
          NewBlocks.Add(Block);
    ...
    begin
      ...
      if Result then
        Schema := FSchemas.Objects[Index] as TXMPSchema;
    end;
    ...
    begin
      NeedLazyLoadedData;
      if Value = FAboutAttributeValue then
        Exit;
    ...
    procedure TXMPPacket.SaveToStream(Stream: TStream);
      ...
      procedure WriteProps(Schema: TXMPSchema; Level: Integer; const Props: IXMPPropertyCollection);
      const
      ...
      begin
        ...
        begin
          ...
          if IsArrayElem or Prop.ParentNamespace then
            ...
            NSDecl := UnicodeFormat(' xmlns:%s="%s"', [Prop.NamespaceInfo.Prefix, Prop.NamespaceInfo.URI]);
          ...
          case Prop.Kind of
            xpStructure:
              RDFName := RDF.DescriptionNodeName;
            xpAltArray:
              RDFName := RDF.AltNodeName;
            xpBagArray:
              RDFName := RDF.BagNodeName;
            xpSeqArray:
              RDFName := RDF.SeqNodeName;
          else
            Assert(False);
          end;
    ...
    const
      PacketStart: UTF8String = '<?xpacket begin="" id="W5M0MpCehiHzreSzNTczkc9d"?>'#10 + '<x:xmpmeta xmlns:x="adobe:ns:meta/" CCRExifVersion="' + CCRExifVersion + '">'#10 +
        #9'<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">'#10;
      PacketEnd: UTF8String = #9'</rdf:RDF>'#10 + '</x:xmpmeta>'#10 + '<?xpacket end="w"?> '; // The packet wrapper (i.e., the <?xpacket lines) are optional according to the XMP spec,
      ...
      DescNodeEnd: UTF8String = #9#9'</rdf:Description>'#10;
    var
      DataPtr: PAnsiChar;
      DataSize: Integer;
      Schema: TXMPSchema;
    begin
      ...
      begin
        ...
        if (DataSize >= SizeOf(TJPEGSegment.XMPHeader)) and CompareMem(DataPtr, @TJPEGSegment.XMPHeader, SizeOf(TJPEGSegment.XMPHeader)) then
        begin
      ...
      begin
        Stream.WriteUTF8Chars(DescNodeStart, [AboutAttributeValue, Schema.NamespaceInfo.Prefix, Schema.NamespaceInfo.URI]);
    ...
    begin
      ...
      begin
        if (SchemaNode.nodeType = ELEMENT_NODE) and (SchemaNode.namespaceURI = RDF.URI) and UnicodeSameText(SchemaNode.nodeName, RDF.DescriptionNodeName) then
        begin
    ...
    procedure TXMPPacket.RemoveProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString);
    var
      Schema: TXMPSchema;
    begin
      if FindSchema(SchemaKind, Schema) then
        Schema.RemoveProperty(PropName);
    end;

    procedure TXMPPacket.RemoveProperties(SchemaKind: TXMPKnownNamespace; const PropNames: array of UnicodeString);
    var
      Schema: TXMPSchema;
    begin
      if FindSchema(SchemaKind, Schema) then
        Schema.RemoveProperties(PropNames);
    end;

    procedure TXMPPacket.DoUpdateProperty(Policy: TXMPWritePolicy; SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; PropKind: TXMPPropertyKind; const NewValue: UnicodeString);
    var
      Prop: TXMPProperty;
      Schema: TXMPSchema;
    begin
      ...
      else if not FindSchema(SchemaKind, Schema) then
        Exit;
    ...
    procedure TXMPPacket.DoUpdateArrayProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; ArrayPropKind: TXMPPropertyKind; const NewValues: array of UnicodeString);
    var
    ...
    procedure TXMPPacket.UpdateProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; PropKind: TXMPPropertyKind; const NewValue: UnicodeString);
    var
      SubPolicy: TXMPWritePolicy;
    begin
      DoUpdateProperty(UpdatePolicy, SchemaKind, PropName, PropKind, NewValue);
      if (SchemaKind <> xsTIFF) or (PropKind <> xpSimple) or (PropName[1] <> UpCase(PropName[1])) then
        Exit;
      ...
      if SubPolicy = xwAlwaysUpdate then
        SubPolicy := xwUpdateIfExists;
      DoUpdateProperty(SubPolicy, SchemaKind, LowerCase(PropName), PropKind, NewValue);
    end;

    procedure TXMPPacket.UpdateBagProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValues: array of UnicodeString);
    begin
      DoUpdateArrayProperty(SchemaKind, PropName, xpBagArray, NewValues);
    end;

    procedure TXMPPacket.UpdateBagProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValueList: UnicodeString);
    begin
      UpdateProperty(SchemaKind, PropName, xpBagArray, NewValueList);
    end;

    procedure TXMPPacket.UpdateSeqProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValues: array of UnicodeString);
    begin
      DoUpdateArrayProperty(SchemaKind, PropName, xpSeqArray, NewValues);
    end;

    procedure TXMPPacket.UpdateSeqProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValueList: UnicodeString);
    begin
      UpdateProperty(SchemaKind, PropName, xpSeqArray, NewValueList);
    end;

    procedure TXMPPacket.UpdateProperty(SchemaKind: TXMPKnownNamespace; const PropName, NewValue: UnicodeString);
    begin
      UpdateProperty(SchemaKind, PropName, xpSimple, NewValue);
    end;

    procedure TXMPPacket.UpdateProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValue: Integer);
    begin
      UpdateProperty(SchemaKind, PropName, xpSimple, IntToStr(NewValue));
    end;

    procedure TXMPPacket.UpdateDateTimeProperty(SchemaKind: TXMPKnownNamespace; const PropName: UnicodeString; const NewValue: TDateTimeTagValue; ApplyLocalBias: Boolean);
    var
      S: UnicodeString;
    begin
